...
function stfly() {
  for (i = 0; i != message.length; i++) {
    if (message.charAt(i) != '$')
      txt +=
        "<span style='position:relative;visibility:hidden;' id='n" +
        i +
        "'>" +
        message.charAt(i) +
        '</span>';
...
function fly2b() {
  if (num4 != message.length) {
    if (message.charAt(num4) != '$') {
      ...
      fly3(
        then.id,
        parseInt(then.style.left),
        parseInt(then.style.left) / 5,
        parseInt(then.style.top),
...
function fly3(target, lef2, num2, top2, num3) {
  if (
    (Math.floor(top2) != 0 && Math.floor(top2) != -1) ||
